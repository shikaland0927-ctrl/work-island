...
- Tune the expanded Liquid Glass notch with persisted Frost, Blur, Refraction, and Bezel Depth sliders while keeping its corner radius fixed; Settings opens a live notch preview, and Reset restores the reference defaults
...
The current source targets local-development version 0.11.21 build 73. The installed `/Applications/Work Island.app` must be compared with staging after each rebuild; rebuilding `dist.noindex` alone does not update the installed copy.
...
For TestFlight or the Mac App Store, open WorkIsland.xcodeproj and use the Work Island scheme. The Store target uses the permanent bundle identifier `com.shikazeriku.workisland`, version 1.0.0 build 57, App Sandbox, and universal arm64/x86_64 release builds. The project supports macOS 13 and later.
