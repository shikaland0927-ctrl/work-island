...
- The main Window's content layer remains Classic: background, cards, sidebar, lists, fields, status, chips, and repeated row controls do not become glass. On macOS 26+, only deliberate control-layer surfaces use official native Glass: Dashboard period selections and arrows, Routine schedule selections, shared opt-in primary/secondary action buttons, and the one-time notch introduction banner. Settings and History segmented choices remain Classic. These Window controls are independent of `Notch Style`, use the existing Classic fallback on older macOS, and must remain outside periodic `TimelineView` content. The persisted notch appearance raw value remains `liquidGlass` for compatibility, but user-facing copy is `Glass`.
