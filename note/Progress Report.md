...
- `Notch Style` still changes only the expanded notch. The Window does not read that preference, but on macOS 26+ a small fixed set of control-layer surfaces always uses official native Glass: selected Dashboard periods, selected Routine schedule choices, Settings segmented choices, shared opt-in primary/secondary actions, and the one-time notch introduction banner. Dashboard period arrows use SwiftUI's native `.navigation` `ControlGroup`, which adopts the current system appearance independently of `Notch Style`. Older macOS uses the existing Classic fallbacks where applicable.
- The shared segmented control keeps its still outer track, labels, and damped spring. Dashboard period, Routine schedule, and Settings selections opt into the same restrained indigo-tinted interactive Glass surface; History Start/End deliberately retains the Classic selected surface.
...
- Graph and Distribution keep their chevron symbols inside SwiftUI's native `.navigation` `ControlGroup`, with a 64-point reserved layout slot. The right arrow remains present but disabled at the current period, matching the native back/forward control behavior.
...
- Activities can be created, renamed, drag-reordered, archived, restored, and deleted with confirmation. Archived rows emit a deletion request to the stable Activities page; the alert and captured Activity ID no longer live inside a List row that can move or be recreated during scrolling.
...
- General: Notch Style, Launch at Login, Day Starts, and Open Notch mode. Notch Style retains the existing `appearanceStyle` key, displays `Classic | Glass`, and controls only the expanded notch. Notch Style and Open Notch use the same Glass-selected segmented surface as Dashboard periods. Blur, optical parameters, and Notch Speed have no Settings sections or rows.
- Timing: Alert, compact Progress, Manual Set As Start/End, shared Timer/Manual Step, and Pomodoro Focus/Break/Long/Sessions. Alert and Manual Set As use the same Glass-selected segmented surface as Dashboard periods. Step defaults to `5 min`, reuses the compatible `manualMinuteStep` key, and does not rewrite Timer or Manual's current duration when changed. The Pomodoro fields use one shared trailing-aligned native `NSPopUpButton` subtree; the standard macOS bezels themselves fill the same 76-point columns with equal 10-point visible gaps.
